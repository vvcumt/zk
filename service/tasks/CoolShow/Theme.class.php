...
class Theme extends CoolShow
{
	...
	private function _getCondition()
	{
		...
		$strType = '';
		if($this->_nType != 0 ){
			$strType =  sprintf(' AND subtype =  %d ', $this->_nType);
		}
		
		$strCharge = $this->getCharge();
		$strCondition = $strType.$strCharge.$strKernel.$this->strPayCondition.$strCharge.$strChoice;
		return $strCondition;
	}
