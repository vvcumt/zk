...
class CoolShowSearch
{
	...
	private function _getDb()
	{
		if(!$this->_db){
			...
			$this->_db 		  = new CoolShowDb($g_arr_db_config['coolshow']);
		}
		return $this->_db;
	}
	...
	private function _setCoolShowParam(CoolShow $coolshow)
	{
		...
		$nVercode  = (int)(isset($_GET['vercode'])?$_GET['verode']:0);#默认值不可变，防止最初版本（未增加此字段的版本）异常
		$nCharge   = (int)(isset($_GET['charge'])?$_GET['charge']:2); #0:免费 1：付费  2：全部  20150504
		$nProtocolCode  = (int)(isset($_GET['procode'])?$_GET['procode']:0);
		
		$coolshow->setParam($nType,
							$nWidth, $nHeight,
						    $nSort,
...
// 				Log::write('CoolShowSearch::getChoiceWallpaer():getSearchResult()'.$strSql, 'log');
	...
	public function getHotWord($nCoolType)
	{
		try {
			$hw = new HotWord();
				
			$strSql = $hw->getSelectHotWordSql($nCoolType);
			if(!$strSql){
